...
class valuetrackerovertime extends utils.Adapter {
    ...
    async _history_fill_history(oS, ts, TimeFrame, isTimeFrameChange) {
        if (oS.history_fill_history_enabled[TimeFrame] && oS.historyReadoutData.lastGoodhis) {
            const val =
                (oS.historyReadoutData.lastGoodhis.val - oS.historyReadoutData.hisstartvalues[TimeFrame]) *
                oS.output_multiplier;
            if (
                isTimeFrameChange ||
                Math.abs(val - oS.historyReadoutData.lastwriteValues[TimeFrame]) >=
                    oS.history_fill_history_MinChange[TimeFrame]
            ) {
    ...
    async _history_readDataFromHistory(oS) {
        ...
        this.log.info(
            `HistoryAnalyse ${oS.id}: Query from Storage Instance  '${
                oS.history_Instanz
            }' all Data (if not working after this please check the instance is active) `,
        );
    ...
    async _history_TimeFrameDetection(oS, TimeFrame, MyHisDate) {
        if (oS.historyReadoutData.lastGoodhis && oS.historyReadoutData.lastHis) {
            if (
                MyHisDate.getDate() == oS.historyReadoutData.lastHis.date.getDate() &&
                MyHisDate.getFullYear() == oS.historyReadoutData.lastHis.date.getFullYear() &&
                MyHisDate.getMonth() == oS.historyReadoutData.lastHis.date.getMonth()
            ) {
            ...
            while (startForOldTimeframe < startForMyHisDate) {
                ...
                if (oS.history_fill_before) {
                    ...
                    oS.historyReadoutData.timeFrameValueData[TimeFrame].push({
                        date: new Date(startForOldTimeframe),
    ...
    async _history_DoAnalyseHistoryData(oS, historyDataList) {
        ...
        for (const TimeFrame in TimeFrames) {
            ...
            oS.history_fill_history_DPs[TimeFrame] =
                `${this.namespace}.${oS.alias}${await this._getObjectIDCurrent(TimeFrame)}`;
            ...
            if (oS.history_fill_history_enabled[TimeFrame]) {
                ...
                if (
                    !(
                        curobj &&
                        curobj.common.custom &&
                        curobj.common.custom[oS.history_fill_history_Instanz] &&
                        curobj.common.custom[oS.history_fill_history_Instanz].enabled
                    )
                ) {
                    this.log.warn(
                        `history_fill: ${oS.history_fill_history_DPs[TimeFrame]} history should be filled, but Instance ${oS.history_fill_history_Instanz} is not enabled`,
                    );
            ...
            if (oS.history_fill_history_enabled[TimeFrame] && oS.history_fill_history_MinChange[TimeFrame] == 0) {
                ...
                if (
                    ...
                    curobj.common.custom &&
                    curobj.common.custom[oS.history_fill_history_Instanz] &&
                    curobj.common.custom[oS.history_fill_history_Instanz].changesMinDelta
                ) {
                    oS.history_fill_history_MinChange[TimeFrame] =
                        curobj.common.custom[oS.history_fill_history_Instanz].changesMinDelta;
                    this.log.info(
                        `readout Minchange ${oS.history_fill_history_MinChange[TimeFrame]} from dp ${oS.history_fill_history_DPs[TimeFrame]}`,
                    );
                }
            }
            ...
            await this.sendToAsync(oS.history_Instanz, 'deleteRange', [
                {
                    id: oS.history_fill_history_DPs[TimeFrame],
                    start: historyDataList[0].date.getTime(),
        ...
        for (const zahler in historyDataList) {
            ...
            if (oS.counterResetDetection && myHis.val < oS.historyReadoutData.lastGoodhis.val) {
                ...
                if (
                    oS.historyReadoutData.counterResetDetetion_CurrentCountAfterReset <=
                    oS.counterResetDetetion_CountAfterReset
                ) {
                    //return;
                } else {
                    const theAnpassung =
                        oS.historyReadoutData.lastGoodhis.val - oS.historyReadoutData.firstWronghis.val;

                    this.log.info(
                        `HistoryAnalyse ${oS.id}: Counter reset! Reset from ${
                            oS.historyReadoutData.lastGoodhis.val
                        } to ${
                            oS.historyReadoutData.firstWronghis.val
                        } (${oS.historyReadoutData.firstWronghis.date.toJSON()}), recalculate start values`,
                    );
                    oS.historyReadoutData.lastGoodhis = myHis;
                    oS.historyReadoutData.firstWronghis = null;
                    oS.historyReadoutData.resetsDetected++;
                    for (const TimeFrame in TimeFrames) {
                        oS.historyReadoutData.hisstartvalues[TimeFrame] =
                            oS.historyReadoutData.hisstartvalues[TimeFrame] - theAnpassung;
                    }
                }
            } else {
                oS.historyReadoutData.firstWronghis = null;
            ...
            if (writenextLog < Date.now()) {
                ...
                this.log.info(
                    `HistoryAnalyse ${oS.id} working state ${Math.round(
                        (oS.historyReadoutData.workcounts.analysed / historyDataList.length) * 100,
                    )}% ` +
                        `; Resets detected: ${oS.historyReadoutData.resetsDetected}; History Entrys Analysed: ${
                            oS.historyReadoutData.workcounts.analysed
                        } (${Math.round(
                            (oS.historyReadoutData.workcounts.analysed -
                                oS.historyReadoutData.workcounts.analysed_last) /
                                zeitSeitlastlogS,
                        )} pro s)` +
                        `; detailed write: ${oS.historyReadoutData.workcounts.detailed} (${Math.round(
                            (oS.historyReadoutData.workcounts.detailed -
                                oS.historyReadoutData.workcounts.detailed_last) /
                                zeitSeitlastlogS,
                        )} pro s)` +
                        `; HistoryWrite: ${oS.historyReadoutData.workcounts.historyWrite} (${Math.round(
                            (oS.historyReadoutData.workcounts.historyWrite -
                                oS.historyReadoutData.workcounts.historyWrite_last) /
                                zeitSeitlastlogS,
                        )} pro s)` +
                    ...
                    for (let zahler = oS.beforeCount(TimeFrame); zahler >= 1; zahler--) {
                        if (MyTimeFrameData.length >= zahler) {
                            await this._pushNewPreviousSates(
                                oS,
                                TimeFrame,
                                MyTimeFrameData[MyTimeFrameData.length - zahler].value,
                                await this._getDateTimeInfoForPrevious(
                                    TimeFrame,
                                    MyTimeFrameData[MyTimeFrameData.length - zahler].date,
                                    0,
                                ),
                            );
                        } else {
        ...
        this.log.info(
            `HistoryAnalyse ${oS.id} FINISHED !! ` +
                ` Resets detected: ${oS.historyReadoutData.resetsDetected} History Entrys Analysed: ${
                    oS.historyReadoutData.workcounts.analysed
                } (${Math.round(oS.historyReadoutData.workcounts.analysed / ZeitGesamtS)} pro s)` +
                ` detailed write: ${oS.historyReadoutData.workcounts.detailed} (${Math.round(
                    oS.historyReadoutData.workcounts.detailed / ZeitGesamtS,
                )} pro s)` +
                ` HistoryWrite: ${oS.historyReadoutData.workcounts.historyWrite} (${Math.round(
                    oS.historyReadoutData.workcounts.historyWrite / ZeitGesamtS,
                )} pro s)`,
        );
    }
}
